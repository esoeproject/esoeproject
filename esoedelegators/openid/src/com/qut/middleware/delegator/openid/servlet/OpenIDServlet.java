 ...
 */
package com.qut.middleware.delegator.openid.servlet;
...
public class OpenIDServlet extends HttpServlet
{
	...
	private Logger logger = LoggerFactory.getLogger(OpenIDServlet.class.getName());
	...
	public void init(ServletConfig servletConfig) throws ServletException
	{
		...
		try
		{
			configFile = new FileInputStream(System.getProperty("openiddeleg.data") + ConfigurationConstants.DELEGATOR_CONFIG);
			...
			this.authnProcessor = (AuthnProcessor) webAppContext.getBean(ConfigurationConstants.AUTHN_PROCESSOR, com.qut.middleware.delegator.openid.authn.AuthnProcessor.class);
		...
		{
			this.logger.error("Exception state encountered while attempting to resolve bean for authnProcessor identified as " + ConfigurationConstants.AUTHN_PROCESSOR);
			throw new ServletException("Exception state encountered while attempting to resolve bean for authnProcessor identified as " + ConfigurationConstants.AUTHN_PROCESSOR, e);
		}
		catch (MalformedURLException e)
		{
			this.logger.error("Unable to locate configuration properties file in servlet context path named " + ConfigurationConstants.DELEGATOR_CONFIG);
			throw new ServletException("Unable to locate configuration properties file in servlet context path named " + ConfigurationConstants.DELEGATOR_CONFIG, e);
		}
		catch (IllegalStateException e)
		{
			this.logger.error("Spring configuration is not in an appropriate state to resolve configured beans");
			throw new ServletException("Spring configuration is not in an appropriate state to resolve configured beans");
		}
		catch (IOException e)
		{
			this.logger.error("IOException while attempting to load configuration properties for shibboleth delegator");
			throw new ServletException("IOException while attempting to load configuration properties for shibboleth delegator", e);
		}
	}
	...
	private void execAuthnProcessor(HttpServletRequest request, HttpServletResponse response) throws ServletException
	{
		...
		/* Determine if user has just accepted/denied the local attribute release acknowledgement policy, if not pass off to openID handler */ 
		...
		if (acceptedPolicy != null && acceptedPolicy.equals(ConfigurationConstants.ACCEPTED_POLICY))
		{
			...
			this.logger.info("Remote session was accepted for establishment by principal identified by: " + data.getSessionID() + " at local epoch timestamp of: " + System.currentTimeMillis());
		...
		if (acceptedPolicy != null && acceptedPolicy.equals(ConfigurationConstants.DENIED_POLICY))
		{
			...
			data.setHttpRequest(request);
			data.setHttpResponse(response);
			this.logger.info("Remote session was not accepted for establishment by principal identified by: " + data.getSessionID() + " denying any further actions");
			try
			{
	...
	{
		this.logger.debug("Setting session cookie with values of - Domain: " + this.sessionDomain + " Name: " + this.sessionTokenName + " value: " + data.getSessionID());
