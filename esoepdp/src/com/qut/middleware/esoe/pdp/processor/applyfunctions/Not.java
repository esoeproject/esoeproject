...
public class Not
{
		...
		public static String FUNCTION_NAME = "not";
		 ...
		 * @return True IFF all child nodes of the given ApplyType evaluate to False, else returns False.
		 */
		...
		public boolean evaluateExpression(JAXBElement<ApplyType> node, Map<String, List<String>> principalAttributes)
		{
			...
			{
				throw new IllegalArgumentException("An Invalid Element was passed to Not.evaluateExpression(). Only Apply Elements can be evaluated as Expressions."); //$NON-NLS-1$
			}
			...
			if (function == null || !function.equals(FUNCTION_NAME))
				throw new IllegalArgumentException(MessageFormat.format("An Invalid Element was passed to Not.evaluateExpression().The Apply element is not an '{0}' function.", FUNCTION_NAME)); //$NON-NLS-1$
