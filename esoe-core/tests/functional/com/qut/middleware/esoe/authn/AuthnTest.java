...
{
	private String LDAP_SERVER = "nettle.qut.edu.au"; 
	...
	public void setUp()
	{
		...
		authenticator = new LdapBasicAuthenticator(this.LDAP_SERVER, this.LDAP_SERVER_PORT, this.BASE_DN, this.IDENTIFIER, false, true);
	...
	public void testExecute()
	{
		...
		expect(request.getParameter(this.FORM_USER_IDENTIFIER)).andReturn("beddoes").anyTimes();
		expect(request.getParameter(this.FORM_PASSWORD_IDENTIFIER)).andReturn("itscandyyoulikeit").anyTimes();
		...
		try
		{
			expect(create.createLocalSession(eq("12345"), eq("beddoes"), eq(AuthenticationContextConstants.passwordProtectedTransport), (List)isNull())).andReturn(Create.result.SessionCreated).anyTimes();
	...
	public void testExecute1() throws Exception
	{
		...
		expect(request.getMethod()).andReturn(this.METHOD_POST).anyTimes();
		expect(request.getParameter(this.FORM_USER_IDENTIFIER)).andReturn("beddoes").anyTimes();
		expect(request.getParameter(this.FORM_PASSWORD_IDENTIFIER)).andReturn("itscandyyoudont'likeit").anyTimes();
		expect(request.getParameter(this.FORM_RESPONSE_IDENTIFIER)).andReturn(null).anyTimes();
		expect(sessionsProcessor.getCreate()).andReturn(create).anyTimes();
		expect(sessionsProcessor.getQuery()).andReturn(query);
		expect(data.getSessionID()).andReturn("12345").anyTimes();
