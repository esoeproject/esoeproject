...
#include "constants/NameIDFormatConstants.h"
#include "constants/VersionConstants.h"
#include "constants/StatusCodeConstants.h"

#include "exceptions/AuthnException.h"
...
void spep::AuthnProcessor::processAuthnResponse( spep::AuthnProcessorData &data )
{
	...
	for( assertionIterator = response->Assertion().begin(); assertionIterator != response->Assertion().end(); assertionIterator++ )
	{
		...
		saml2::protocol::ResponseType::Assertion::type::AuthnStatement::const_iterator authnStatementIterator;
...
void spep::AuthnProcessor::logoutPrincipal( spep::AuthnProcessorData &data )
{
	
	std::auto_ptr<saml2::protocol::LogoutRequestType> logoutRequest;
	PrincipalSession principalSession;
	...
	try
	{
		logoutRequest.reset( _logoutRequestUnmarshaller->unMarshallSigned( data.getRequestDocument() ) );

		//_samlValidator->getRequestValidator()->validate( logoutRequest );
		...
		else
		{
			...
			data.setResponseDocument( generateLogoutResponse( saml2::statuscode::REQUESTOR, L"No NameID was specified to be terminated.", requestSAMLID ) );
		}
	
	}
	catch (saml2::UnmarshallerException &ex)
	{
		this->_localReportingProcessor.log( ERROR, "Failed to unmarshal logout request. Message was: " + ex.getMessage() + ". Cause was: " + ex.getCause() );
		
		// Can't return null, no pointer type
		data.setResponseDocument( generateLogoutResponse( saml2::statuscode::REQUESTOR, L"Unable to unmarshal the supplied logout request document" ) );
		return;
	}
	catch (std::exception &ex)
	{
		this->_localReportingProcessor.log( ERROR, std::string("Unknown error trying to unmarshal logout request. Message was: ") + ex.what() );
	...
	data.setResponseDocument( generateLogoutResponse( saml2::statuscode::SUCCESS, L"Logout succeeded" ) );
	
}

saml2::SAMLDocument spep::AuthnProcessor::generateLogoutResponse( const std::wstring &statusCodeValue, const std::wstring &statusMessage, const std::wstring &inResponseTo )
{
	...
	try
	{
		return this->_logoutResponseMarshaller->marshallSigned( &response, idList );
	}
	catch( saml2::MarshallerException &ex )
	{
