 ...
 */
package com.qut.middleware.esoe.authn.plugins.usernamepassword.authenticator;
...
public class LdapBasicAuthenticator implements UserPassAuthenticator
{
	...
	private Logger logger = LoggerFactory.getLogger(LdapBasicAuthenticator.class.getName());
			...
			boolean recursive, boolean disableSSL)
	{
		...
		if (ldapServer == null)
		{
			this.logger.error(Messages.getString("LdapBasicAuthenticator.0")); //$NON-NLS-1$
			throw new IllegalArgumentException(Messages.getString("LdapBasicAuthenticator.0")); //$NON-NLS-1$
		}

		if (baseDN == null)
		{
			this.logger.error(Messages.getString("LdapBasicAuthenticator.1")); //$NON-NLS-1$
			throw new IllegalArgumentException(Messages.getString("LdapBasicAuthenticator.1")); //$NON-NLS-1$
		}

		if (identifier == null)
		{
			this.logger.error(Messages.getString("LdapBasicAuthenticator.2")); //$NON-NLS-1$
