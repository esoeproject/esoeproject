...
public class PolicyCacheProcessorImpl extends Thread implements PolicyCacheProcessor, MonitorThread
{
	 ...
	 * 
	 */
	public PolicyCacheProcessorImpl(AuthzCacheUpdateFailureRepository failureRep, AuthzPolicyCache cache, Metadata metadata, PolicyCacheDao sqlConfig, WSClient wsClient, KeyStoreResolver keyStoreResolver, IdentifierGenerator identifierGenerator, SAMLValidator samlValidator, int pollInterval) throws MarshallerException, UnmarshallerException
	{
	...
	public result spepStartingNotification(String entityID, int authzCacheIndex)
	{
		...
		try
		{
			...
			byte[] authzClearCacheRequest = this.generateClearCacheRequest(entityID, endpoint, Messages.getString("PolicyCacheProcessorImpl.3")); //$NON-NLS-1$
		...
		while (entityIter.hasNext())
		{
			...
			try
			{
				...
				while (endpointIter.hasNext())
				{
					...
					authzClearCacheRequest = this.generateClearCacheRequest(entityID, endpoint, Messages.getString("PolicyCacheProcessorImpl.4")); //$NON-NLS-1$
	 ...
	 * deliver to.
	 */
	private void recordFailure(byte[] request, String endPoint)
	{
