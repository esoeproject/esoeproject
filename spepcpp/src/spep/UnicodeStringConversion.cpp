...
UnicodeString spep::UnicodeStringConversion::toUnicodeString( const unsigned char *src, int srcLength )
{
	...
	class AutoUConverter
	{
		private:
		UConverter *_cnv;
		
		public:
		AutoUConverter( const char *converterName, UErrorCode *err )
		:
		_cnv( ucnv_open( converterName, err ) )
		{}
	...
	AutoUConverter sourceConverter( unicodeType, &error );
	...
	UnicodeString result( documentStart, documentLength, sourceConverter.get(), error );
