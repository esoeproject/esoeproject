 ...
 */
package com.qut.middleware.tools.war.logic;
...
{
	/*
	 * NB: Good class name eh? Perhaps I should have called this SteveBalmerSaysOpenSourceSucksLogic ;-)
	 */
	...
	private void generateWarContents(String parentDir, File dir, JarOutputStream out) throws IOException
	{
		...
		for (File jarFile : jarContents)
		{
			...
			{
				if (parentDir != null)
					generateWarContents(parentDir + File.separator + jarFile.getName(), jarFile, out);
				else
			...
			JarEntry jarAdd;
			if (parentDir != null)
				jarAdd = new JarEntry(parentDir + File.separator + jarFile.getName());
			else
	...
	private void copyFiles(File src, File dest) throws IOException
	{
		...
		for (File file : srcList)
		{
			if (file.isDirectory())
			{
				File recDest = new File(dest.getAbsolutePath() + File.separator + file.getName());
				File recSrc = new File(src.getAbsolutePath() + File.separator + file.getName());
	...
	private void writeAdditionalContent(AdditionalContent content, File dest) throws IOException
	{
		byte buffer[] = new byte[10000];
		...
		try
		{
			...
			out = new FileOutputStream(dest + File.separator + content.getPath());
			while (true)
			{
				int nRead = in.read(buffer, 0, buffer.length);
				if (nRead <= 0)
	...
	public void generateWar(List<AdditionalContent> additionalContent, File srcDir, File outDir, File warFile) throws GenerateWarException
	{
		...
		finally
		{
			...
			catch (IOException e)
			{
				...
				this.logger.error("Unable to close streams correctly " + e.getLocalizedMessage());
			}
		}
	}
}
