...
#include "exceptions/AttributeException.h"
...
void spep::AttributeProcessor::doAttributeProcessing(spep::PrincipalSession &principalSession)
{
	...
	this->buildAttributeQuery( data, principalSession, samlID );
	
	if (data.getRequestDocument().getData() == NULL || data.getRequestDocument().getLength() == 0)
	{
		throw AttributeException( "The request document was null or zero length. Aborting attribute query." );
	}
	
	// Perform the query and get the response document.
	...
	try
	{
		...
		this->_wsClient->attributeAuthority( wsData, endpoint );
	}
	...
	try
	{
		...
		this->processAttributeResponse( data, principalSession, samlID );
	}
	catch ( std::exception& ex )
	{
...
void spep::AttributeProcessor::buildAttributeQuery( spep::AttributeProcessorData &data, spep::PrincipalSession &principalSession, const std::wstring &samlID )
{
	...
	try
	{
		...
		// Marshal the request.
		saml2::SAMLDocument samlDocument( this->_attributeQueryMarshaller->marshallSigned( &attributeQuery, identifierList ) );
		
		data.setRequestDocument( samlDocument );
...
void spep::AttributeProcessor::processAttributeResponse( spep::AttributeProcessorData &data, spep::PrincipalSession &principalSession, const std::wstring &samlID )
{
	if ( data.getResponseDocument().getData() == NULL || data.getResponseDocument().getLength() == 0 )
	{
	...
	try
	{
		response.reset( this->_responseUnmarshaller->unMarshallSigned( data.getResponseDocument(), true ) );
	}
	catch( saml2::UnmarshallerException &ex )
	{
		this->_localReportingProcessor.log( ERROR, "Failed to unmarshal attribute response document. Exception was: " + ex.getMessage() + ". Cause was: " + ex.getCause() );
		throw AttributeException( "Failed to unmarshal attribute response document. Couldn't resolve attributes for session." );
	}
	
	this->_localReportingProcessor.log( DEBUG, "Unmarshalled attribute response document. Gathering attribute statements." );
		...
		++assertionIterator )
	{
		...
		saml2::assertion::AssertionType::AttributeStatement::iterator attributeStatementIterator;
...
void spep::AttributeProcessor::processAttributeStatements( AttributeStatementPointerList &attributeStatements, spep::PrincipalSession &principalSession )
{
		...
		++attributeStatementIterator )
	{
		...
		saml2::assertion::AttributeStatementType::Attribute::iterator attributeIterator;
			...
			++attributeIterator )
		{
			...
			saml2::assertion::AttributeType::AttributeValue::iterator attributeValueIterator;
