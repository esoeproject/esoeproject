...
#define STATUSCODECONSTANTS_H_
 ...
 namespace saml2
 {
 	namespace statuscode
 	{
		 ...
		 * elements.
		 */
		const static std::wstring SUCCESS= L"urn:oasis:names:tc:SAML:2.0:status:Success";
	
		/** The request could not be performed due to an error on the part of the requester */
		const static std::wstring REQUESTOR= L"urn:oasis:names:tc:SAML:2.0:status:Requester";
	
		/** The request could not be performed due to an error on the part of the responder or SAML authority */
		const static std::wstring RESPONDER= L"urn:oasis:names:tc:SAML:2.0:status:Responder";
	
		/** The SAML responder could not process the request because the version of the request message was incorrect. */
		const static std::wstring VERSION_MISMATCH= L"urn:oasis:names:tc:SAML:2.0:status:VersionMismatch";
		...
		const static std::wstring AUTHN_FAILED= L"urn:oasis:names:tc:SAML:2.0:status:AuthnFailed";
	
		/**
		 * Unexpected or invalid content was encountered within a <saml:Attribute> or <saml:AttributeValue> element.
		 */
		const static std::wstring INVALID_ATTR= L"urn:oasis:names:tc:SAML:2.0:status:InvalidAttrNameOrValue";
	
		/** The responding provider cannot or will not support the requested name identifier policy. */
		const static std::wstring INVALID_NAMEID_POLICY= L"urn:oasis:names:tc:SAML:2.0:status:InvalidNameIDPolicy";
	
		/** The specified authentication context requirements cannot be met by the responder. */
		const static std::wstring NO_AUTHN_CONTENT= L"urn:oasis:names:tc:SAML:2.0:status:NoAuthnContext";
		 ...
		 */
		const static std::wstring NO_AVAILABLE_IDP= L"urn:oasis:names:tc:SAML:2.0:status:NoAvailableIDP";
	
		/** Indicates the responding provider cannot authenticate the principal passively, as has been requested. */
		const static std::wstring NO_PASSIVE= L"urn:oasis:names:tc:SAML:2.0:status:NoPassive";
	
		/**
		 * Used by an intermediary to indicate that none of the identity providers in an <IDPList> are supported by the
		 * intermediary.
		 */
		const static std::wstring NO_SUPPORTED_IDP= L"urn:oasis:names:tc:SAML:2.0:status:NoSupportedIDP";
		 ...
		 */
		const static std::wstring PARTIAL_LOGOUT= L"urn:oasis:names:tc:SAML:2.0:status:PartialLogout";
	
		/**
		 * Indicates that a responding provider cannot authenticate the principal directly and is not permitted to proxy the
		 * request further.
		 */
		const static std::wstring PROXY_COUNT_EXCEEDED= L"urn:oasis:names:tc:SAML:2.0:status:ProxyCountExceeded";
		 ...
		 */
		const static std::wstring REQUEST_DENIED= L"urn:oasis:names:tc:SAML:2.0:status:RequestDenied";
	
		/** The SAML responder or SAML authority does not support the request. */
		const static std::wstring REQUEST_UNSUPPORTED= L"urn:oasis:names:tc:SAML:2.0:status:RequestUnsupported";
	
		/** The SAML responder cannot process any requests with the protocol version specified in the request. */
		const static std::wstring REQUEST_VERSION_DEPRECATED= L"urn:oasis:names:tc:SAML:2.0:status:RequestVersionDeprecated";
		 ...
		 */
		const static std::wstring REQUEST_VERSION_TOO_HIGH= L"urn:oasis:names:tc:SAML:2.0:status:RequestVersionTooHigh";
	
		/**
		 * The SAML responder cannot process the request because the protocol version specified in the request message is
		 * too low.
		 */
		const static std::wstring REQUEST_VERSION_TOO_LOW= L"urn:oasis:names:tc:SAML:2.0:status:RequestVersionTooLow";
	
		/** The resource value provided in the request message is invalid or unrecognized. */
		const static std::wstring RESOURCE_NOT_RECOGNISED= L"urn:oasis:names:tc:SAML:2.0:status:ResourceNotRecognized";
	
		/** The response message would contain more elements than the SAML responder is able to return. */
		const static std::wstring TOO_MANY_RESPONSES= L"urn:oasis:names:tc:SAML:2.0:status:TooManyResponses";
	
		/**
		 * An entity that has no knowledge of a particular attribute profile has been presented with an attribute drawn from
		 * that profile.
		 */
		const static std::wstring UNKNOWN_ATTR_PROFILE= L"urn:oasis:names:tc:SAML:2.0:status:UnknownAttrProfile";
	
		/** The responding provider does not recognize the principal specified or implied by the request. */
		const static std::wstring UNKNOWN_PRINCIPAL= L"urn:oasis:names:tc:SAML:2.0:status:UnknownPrincipal";
	
		/**
		 * The SAML responder cannot properly fulfill the request using the protocol binding specified in the request.
		 */
		const static std::wstring UNSUPPORTED_BINDING= L"urn:oasis:names:tc:SAML:2.0:status:UnsupportedBinding";
 	}
 }
