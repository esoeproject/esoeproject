...
public class WsLogoutMechanismImpl implements LogoutMechanism 
{
	...
	public result performSingleLogout(String samlAuthnID, List<String> sessionIDs, String endpoint, boolean storeFailedLogout) 
	{
		...
		try
		{
			byte[] logoutRequest =this.generateLogoutRequest(samlAuthnID, sessionIDs);
	 ...
	 */
	private LogoutMechanism.result sendLogoutRequest(byte[] logoutRequest, String endPoint, String samlAuthnID, boolean storeFailedLogout)
	{
		...
		try
		{
			try
			{
				this.logger.trace(MessageFormat.format( "Making web service single logout call ..\n{0}", new String(logoutRequest, "UTF-16"))  ); //$NON-NLS-1$
			}
			catch(UnsupportedEncodingException e)
			{
				this.logger.trace("Unsupported Document encoding ..", e);
			}
	 ...
	 */
	private byte[] generateLogoutRequest(String samlAuthnID,  List<String> sessionIndicies) throws MarshallerException
	{
		...
		requestDocument = this.logoutRequestMarshaller.marshallSigned(request);
	 ...
	 */
		...
		{		
			...
			try
			{
				...
				this.logger.trace("Processing FailedLogout Request :\n " + requestString);
			}
			catch(UnsupportedEncodingException e)
			{
				e.printStackTrace();
