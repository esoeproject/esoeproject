...
public class Artifact
{
	...
	public Artifact(int index, byte[] sourceID, byte[] messageHandle, String audience, Element document)
	{
		this.type = TYPE_ID;
		this.index = index;
		...
		{
			throw new IllegalArgumentException("Source ID length must be 20");
		}
		this.sourceID = sourceID;
		
		if (messageHandle.length != 20)
		{
			throw new IllegalArgumentException("Message handle length must be 20");
		}
		this.messageHandle = messageHandle;
		
		if (audience == null)
		{
			throw new IllegalArgumentException("Audience for SAML artifact cannot be null");
		}
		this.audience = audience;
		
		if (document == null)
		{
			throw new IllegalArgumentException("SAML artifact cannot have a null document element");
		}
	...
	public Artifact(String artifact)
	{
		...
		{
			throw new IllegalArgumentException("Unable to parse the SAML artifact token, it is not the correct length required by the SAML spec (20 bytes).");
		}
		...
		if (type != 0x0004)
		{
			throw new IllegalArgumentException("Unable to parse the SAML artifact token, unrecognized type ID: " + type);
		}

		if (rawArtifact.length != 44)
		{
			throw new IllegalArgumentException("Unable to parse the SAML artifact token, it is not the correct length required by the SAML spec for type ID 0x0004 (44 bytes).");
		}
