...
public class AndUI extends OperatorUI
{
	public AndUI(OperatorUI parent, boolean editMode)
	{
	...
	public void createLocalInterface()
	{
		...
		if (children != null && children.size() > 0)
		{
			for (int i = 0; i < children.size(); i++)
			{
				...
				if (children.size() == 1 || (i + 1) != children.size())
				{
					Label orLbl = new Label("AND");
					orLbl.addStyleName(CSSConstants.esoeManagerPolicyOperatorTitleBoolean);
					localContent.add(orLbl);
				}
			}
		}
		else
	...
	public void validate(List<String> idList) throws PolicyComponentValidationException
	{
		...
		{
			this.localMessages.errorMsg("AND operators must have at least two children");
			throw new PolicyComponentValidationException("AND operators must have at least two children");
		}
	...
	public void addChild(OperatorUI child, String childType)
	{
		...
		EventController.executeEvent(new MessageEvent(EventConstants.userMessage, EsoeManagerConstants.areaID,
				MessageEvent.error, "Unable to add " + childType + " to an AND operator"));
	}
