...
#include "exceptions/UnmarshallerException.h"
...
#include "resolver/ExternalKeyResolver.h"
#include "SAML2Defs.h"
...
namespace saml2
{
	...
	class UnmarshallerImpl : public saml2::Unmarshaller<T>
	{
		...
		private:
			...
			void validateSignature(DOMDocument* doc, XSECCryptoKey* pk);
	...
	T* UnmarshallerImpl<T>::unMarshallUnSignedElement (DOMElement* elem, bool keepDOM)
	{
		...
		try
		{
			if(keepDOM)
			{
				 ...
				 * in this case and will be cleaned up as required when this object is removed 
				 */
				/* Not anymore - using the XSD internal flag "not_root" to prevent this. Since we would
				 * never use unMarshallUnSignedElement at the root of a document, deleting the DOM from the
				 * XSD object will be an invalid operation (it will already be deleted by another XSD object)
				 * In addition, this gives us consistent behaviour (from a memory management perspective) for
				 * keepDOM=true and keepDOM=false (i.e. caller is responsible for deleting the DOM object).
				 * - SM
				 */
				obj = new T ( *elem, xsd::cxx::tree::flags::keep_dom | xsd::cxx::tree::flags::not_root | xsd::cxx::tree::flags::dont_validate | xsd::cxx::tree::flags::dont_initialize);
			}
			else
			{
			...
			for(XMLSize_t i = 0; i < keyDescriptorList->getLength(); i++)
			{
				...
				for(XMLSize_t j = 0; j < keyInfoList->getLength(); j++)
				{
					...
					if( RSAKeyDataList->getLength() == 1 )
					{
						...
						/* Create a RSA key from provided data */
						// TODO Is this being leaked?
						key = new OpenSSLCryptoKeyRSA();
					...
					if( DSAKeyDataList->getLength() == 1 )
					{
						...
						/* Create a DSA key from provided data */
						key = new OpenSSLCryptoKeyDSA();
