...
spep::WSClient::WSClient(saml2::Logger *logger, const std::string& caBundle, spep::SOAPUtil *soapUtil) :
    mLocalLogger(logger, "spep::WSClient"),
    mCABundle(caBundle),
    mSoapUtil(soapUtil)
{
}
...
void spep::WSClient::doSOAPRequest(spep::WSProcessorData& data, const std::string& endpoint)
{
    ...
    mLocalLogger.debug() << "Calling cURL to make web service call to " << endpoint;
    ...
    if (!data.getCharacterEncoding().empty())
    {
        contentTypeHeader += "; charset=";
        contentTypeHeader += data.getCharacterEncoding();
    }

    mLocalLogger.debug() << std::string("Setting content type header to - ") << contentTypeHeader;
...
int spep::WSClient::debugCallback(CURL *curl, curl_infotype info, char *msg, std::size_t len, void *userp)
{
    ...
    case CURLINFO_TEXT:
        ...
        wsClient->mLocalLogger.debug() << ss.str();
        break;
