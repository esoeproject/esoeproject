...
{
	private String[] authnSchemas = new String[] { ConfigurationConstants.samlProtocol,
	...
	public AuthnProcessorImpl( AttributeProcessor attributeProcessor,
			...
			IdentifierGenerator identifierGenerator, KeyStoreResolver keyStoreResolver,
			int attributeConsumingServiceIndex, int assertionConsumerServiceIndex) throws MarshallerException,
			UnmarshallerException
	{
		...
		this.authnRequestMarshaller = new MarshallerImpl<AuthnRequest>(this.MAR_PKGNAMES2, this.authnSchemas,
		...
		this.logoutRequestUnmarshaller = new UnmarshallerImpl<LogoutRequest>(this.UNMAR_PKGNAMES, this.logoutSchemas,
				this.metadata);
		this.logoutResponseMarshaller = new MarshallerImpl<Response>(this.MAR_PKGNAMES, this.logoutSchemas,
				keyStoreResolver.getKeyAlias(), keyStoreResolver.getPrivateKey());
	...
	public void generateAuthnRequest(AuthnProcessorData data) throws AuthenticationException
	{
		...
		try
		{
			String requestDocument = this.authnRequestMarshaller.marshallSigned(authnRequest);
	...
	public void processAuthnResponse(AuthnProcessorData data) throws AuthenticationException
	{
		...
		try
		{
			...
			this.logger.debug(MessageFormat.format(
					Messages.getString("AuthnProcessorImpl.19"), response.getID())); //$NON-NLS-1$
		}
		catch (SignatureValueException e)
		{
			this.logger.warn(
					Messages.getString("AuthnProcessorImpl.6") + e.getLocalizedMessage()); //$NON-NLS-1$
			throw new AuthenticationException(Messages.getString("AuthnProcessorImpl.7"), e); //$NON-NLS-1$
		}
		catch (ReferenceValueException e)
		{
			this.logger.warn(
					Messages.getString("AuthnProcessorImpl.8") + e.getLocalizedMessage()); //$NON-NLS-1$
			throw new AuthenticationException(Messages.getString("AuthnProcessorImpl.9"), e); //$NON-NLS-1$
		}
		catch (UnmarshallerException e)
		{
			this.logger.warn(
					Messages.getString("AuthnProcessorImpl.10") + e.getLocalizedMessage()); //$NON-NLS-1$
		...
		catch (InvalidSAMLResponseException e)
		{
			this.logger.warn( MessageFormat.format(
					Messages.getString("AuthnProcessorImpl.52"), response.getID(), e.getMessage())); //$NON-NLS-1$
		}

		// Validate destination to ensure it is for this SPEP.
		String destination = response.getDestination();
		if (!this.metadata.getSPEPAssertionConsumerLocation().equals(destination))
		{
			this.logger.error(MessageFormat.format(Messages.getString("AuthnProcessorImpl.53"), this.metadata.getSPEPAssertionConsumerLocation(), destination)); //$NON-NLS-1$
		...
		for (Object assertionOrEncryptedAssertion : response.getEncryptedAssertionsAndAssertions())
		{
			if (assertionOrEncryptedAssertion instanceof Assertion)
			{
				...
				for (SubjectConfirmation confirmation : subjectConfirmations)
				{
					...
					if (!this.metadata.getSPEPAssertionConsumerLocation().equals(recipient))
					{
						this.logger.error( Messages.getString("AuthnProcessorImpl.68")); //$NON-NLS-1$
						this.logger.debug(MessageFormat.format(Messages.getString("AuthnProcessorImpl.69"), this.metadata.getSPEPAssertionConsumerLocation(), recipient)); //$NON-NLS-1$
				...
				List<ConditionAbstractType> audienceRestrictions = assertion.getConditions()
				...
				for (ConditionAbstractType restriction : audienceRestrictions)
				{
					if (restriction instanceof AudienceRestriction)
					{
						...
						if (!ar.getAudiences().contains(this.metadata.getSPEPAssertionConsumerLocation()))
						{
		...
		this.logger.error( MessageFormat.format(
				Messages.getString("AuthnProcessorImpl.14"), response.getStatus().getStatusMessage())); //$NON-NLS-1$
	...
	public PrincipalSession verifySession(String sessionID)
	{
		...
		{
			this.logger.warn(MessageFormat.format(
					Messages.getString("AuthnProcessorImpl.21"), sessionID)); //$NON-NLS-1$
		}
	...
	public void logoutPrincipal(AuthnProcessorData data) throws LogoutException
	{
		...
		this.logger.debug(MessageFormat.format(
				Messages.getString("AuthnProcessorImpl.30"), samlID)); //$NON-NLS-1$
		...
		try
		{
			if (principalSession != null)
			{
				...
				{
					this.logger.debug(MessageFormat.format(Messages
							.getString("AuthnProcessorImpl.31"), principalSession.getEsoeSessionID())); //$NON-NLS-1$
	...
	{
		this.logger.debug(MessageFormat.format(
				Messages.getString("AuthnProcessorImpl.42"), statusCodeValue, statusMessage)); //$NON-NLS-1$
