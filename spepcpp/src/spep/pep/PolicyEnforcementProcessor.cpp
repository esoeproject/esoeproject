...
void spep::PolicyEnforcementProcessor::makeAuthzDecision( spep::PolicyEnforcementProcessorData &data )
{
	...
	if ( policyDecision == spep::Decision::CACHE )
	{
		...
		try
		{
			response.reset( this->_wsClient->doWSCall( this->_metadata->getAuthzServiceEndpoint(), requestDocument.get(), _responseUnmarshaller ) );
		}
...
XERCES_CPP_NAMESPACE::DOMDocument* spep::PolicyEnforcementProcessor::generateClearAuthzCacheResponse( std::wstring &inResponseTo, std::wstring &statusMessage, std::wstring &statusCodeValue )
{
	...
	DOMDocument *responseDocument = _clearAuthzCacheResponseMarshaller->generateDOMDocument( &response );
...
XERCES_CPP_NAMESPACE::DOMDocument* spep::PolicyEnforcementProcessor::generateAuthzDecisionQuery( spep::PolicyEnforcementProcessorData &data )
{
	...
	try
	{
		DOMDocument *requestDocument = _lxacmlAuthzDecisionQueryMarshaller->generateDOMDocument( &authzQuery );
		requestDocument = _lxacmlAuthzDecisionQueryMarshaller->validate( requestDocument );
		this->_lxacmlAuthzDecisionQueryMarshaller->sign( requestDocument, idList );
