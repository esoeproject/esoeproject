...
namespace saml2
{
	...
	void SAMLAssertionValidator::validate(saml2::assertion::AssertionType* assertion)
	{
		...
		saml2::assertion::SubjectType::SubjectConfirmation::container& confirmations = assertion->Subject().get().SubjectConfirmation();
		
		/* Processes subjectconfirmation fields, at present we only look at Subject confirmation data */
		saml2::assertion::SubjectType::SubjectConfirmation::iterator i = confirmations.begin();
		while ( i != confirmations.end() )
		{
			saml2::assertion::SubjectConfirmationType::SubjectConfirmationData::container confirmationData = i->SubjectConfirmationData();
			if ( confirmationData.present() )
			{
				if( confirmationData->NotOnOrAfter().present() )
				{
					saml2::assertion::SubjectConfirmationDataType::NotOnOrAfter::type xmlTime = confirmationData->NotOnOrAfter().get();
